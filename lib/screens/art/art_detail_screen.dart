...
class _ArtDetailScreenState extends State<ArtDetailScreen> {
  ...
  final TextEditingController _bidAmountController =
      TextEditingController(); // 입찰 금액 입력 컨트롤러
      ...
      if (response.statusCode == 200) {
        ...
        if (postData != null) {
          setState(() {
            ...
            _isSeller =
                _currentUserId != null && artwork?.userId == _currentUserId;
            // 경매 종료 상태일 때만 최고 입찰자인지 확인 (tradeStatus 사용)
            _isSuccessfulBidder =
                _currentUserId != null &&
                artwork?.tradeDTO?.bidderId == _currentUserId &&
                artwork?.tradeDTO?.tradeStatus == true;
  ...
  Future<void> _toggleLikeStatus() async {
    ...
    setState(() {
      ...
      artwork!.favoriteCnt =
          currentLikedStatus ? currentFavoriteCnt - 1 : currentFavoriteCnt + 1;
    });
      ...
      if (response.statusCode == 200) {
        ...
        if (response.body.isNotEmpty) {
          try {
            ...
            errorMessage =
                '$errorMessage: ${response.body}'; // 파싱 실패 시 원본 본문 포함
          }
        }
        ScaffoldMessenger.of(
          context,
  ...
  Future<void> _placeBid() async {
    ...
    if (_currentUserId != null && _currentUserId == artwork?.userId) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('자신의 작품에는 입찰할 수 없습니다.')));
      return;
    }

    // 경매가 종료되었는지 확인 (tradeStatus 사용)
    if (artwork?.tradeDTO?.tradeStatus ?? true) {
      // tradeStatus가 null이거나 true이면 종료로 간주
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('종료된 경매에는 입찰할 수 없습니다.')));
      return;
    }
    ...
    if (instantPurchasePrice != null && bidAmount > instantPurchasePrice) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '입찰가는 즉시 구매가(${instantPurchasePrice.toString().replaceAllMapped(RegExp(r'(?<!\\d)(?:(?=\\d{3})+(?!\\d)|(?<=\\d)(?=(?:\\d{3})+(?!\\d)))'), (m) => ',')}원)를 초과할 수 없습니다.',
          ),
        ),
      );
      return;
          ...
          await showDialog(
            ...
            builder: (BuildContext context) {
              return AlertDialog(
                ...
                content: Text(
                  '현재 지정한 입찰 금액(${bidAmount.toString().replaceAllMapped(RegExp(r'(?<!\\d)(?:(?=\\d{3})+(?!\\d)|(?<=\\d)(?=(?:\\d{3})+(?!\\d)))'), (m) => ',')}원)은 즉시구매가와 동일합니다.\n즉시구매 페이지로 이동하시겠습니까?',
                ),
                actions: <Widget>[
                  TextButton(
                    child: const Text('취소'),
    ...
    if (_currentUserId != null &&
        ...
        previousBidderId == _currentUserId) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('이미 최고 입찰자입니다.')));
      return;
    }

    // 최소 입찰가 검증 (즉시 구매 로직 통과 후에 확인)
    final currentHighestBid =
        artwork?.tradeDTO?.highestBid ?? artwork?.tradeDTO?.startPrice ?? 0;
    final minBidAmount = currentHighestBid + 1000; // 최소 1000원 이상 높게
    if (bidAmount < minBidAmount) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '입찰가는 현재 최고가(${currentHighestBid.toString().replaceAllMapped(RegExp(r'(?<!\\d)(?:(?=\\d{3})+(?!\\d)|(?<=\\d)(?=(?:\\d{3})+(?!\\d)))'), (m) => ',')}원)보다 1000원 이상 높아야 합니다.',
          ),
        ),
      );
      return;
    ...
    try {
      ...
      debugPrint(
        '입찰 API 요청 본문: {"tradeId": $tradeId, "bidAmount": $bidAmount}',
      );

      final response = await http.post(
        uri,
        headers: headers,
        body: jsonEncode({'tradeId': tradeId, 'bidAmount': bidAmount}),
      );
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('입찰 실패: ${e.toString()}')));
    } finally {
      setState(() {
        _isBidding = false;
      });
  ...
  void _handleInstantPurchase() {
    ...
    if (_isSeller) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('자신의 작품을 즉시 구매할 수 없습니다.')));
      return;
    }
    ...
    if (artwork?.tradeDTO?.tradeStatus ?? true) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('종료된 경매는 즉시 구매할 수 없습니다.')));
      return;
    }
  ...
  void _handleChat() {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('채팅 기능은 아직 구현되지 않았습니다.')));
  }
  ...
  void _handleReregisterAuction() {
    if (artwork?.postId != null) {
      Navigator.pushNamed(
        context,
        '/art/register',
        arguments: {'postData': artwork, 'isReregister': true},
      );
    } else {
      debugPrint('재등록할 게시글 정보(postId)가 없습니다.');
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('재등록할 작품 정보를 찾을 수 없습니다.')));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final bool isAuctionInProgress =
        artwork?.tradeDTO != null && (artwork!.tradeDTO!.tradeStatus == false);
    // 경매 종료 여부 (tradeStatus가 true일 때 종료)
    final bool isAuctionEndedBasedOnStatus =
        artwork?.tradeDTO?.tradeStatus == true;

    // 즉시구매 버튼을 보여줄지 결정: 경매 진행 중이고 판매자가 아닐 때
    final bool showInstantPurchaseButton =
        isAuctionInProgress &&
        !_isSeller &&
        artwork?.tradeDTO?.nowBuy != null &&
        (artwork?.tradeDTO?.nowBuy ?? 0) > 0; // TradeDTO 모델 속성 접근
    // 입찰 관련 UI를 보여줄지 결정: 경매 진행 중이고 판매자가 아닐 때
    final bool showBidSection =
        isAuctionInProgress && !_isSeller && artwork?.tradeDTO != null;
    // 경매 종료 후 작가와 채팅 버튼을 보여줄지 결정: 경매 종료 && 낙찰자인 경우
    final bool showChatAfterEnded =
        isAuctionEndedBasedOnStatus && _isSuccessfulBidder;
    // 경매 진행 중일 때 작가와 채팅 버튼을 보여줄지 결정: 경매 진행 중 && 판매자가 아닌 경우
    final bool showChatDuringAuction = isAuctionInProgress && !_isSeller;

    return Scaffold(
      appBar: AppBar(title: Text(artwork?.title ?? '상세 정보')),
      body:
          isLoading
                          ...
                          child: GestureDetector(
                            ...
                            child: Row(
                              children: [
                                CircleAvatar(
                                  backgroundImage:
                                      artwork!.profileImage != null
                                          ? NetworkImage(
                                            // React 코드의 profileImage 처리 로직 참고
                                            artwork!.profileImage!.startsWith(
                                                  '/ourlog',
                                                )
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          IconButton(
                            ...
                            onPressed:
                                fetchArtworkDetails, // 누르면 fetchArtworkDetails 호출
                          ),
                        ],
                      ),
                      const SizedBox(height: 8),
                        ...
                        child: Row(
                          children: [
                            ...
                            Text(
                              ...
                              isAuctionEndedBasedOnStatus ? '경매 종료' : countdown,
                              // 경매 종료 여부에 따라 텍스트 표시
                              style: Theme.of(
                                context,
                              ).textTheme.titleMedium?.copyWith(
                          ...
                          children: [
                            ...
                            if (showInstantPurchaseButton)
                              Expanded(
                                child: ElevatedButton(
                                  onPressed:
                                      _handleInstantPurchase, // 즉시구매 로직 연결
